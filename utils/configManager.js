...
async function loadCommands(client) {
  ...
  try {
    // Registrar instantaneamente en el servidor de pruebas
    ...
    // Registrar globalmente tambien (puede tardar hasta 1h en otros servidores)
    console.log("Registrando " + commands.length + " comandos globalmente...");
    const globalData = await rest.put(
      Routes.applicationCommands(CLIENT_ID),
      { body: commands }
    );
    console.log(globalData.length + " comandos registrados globalmente (pueden tardar hasta 1h)");
